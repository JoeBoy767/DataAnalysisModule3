...
-- This depends on what is being asked for here:
-- If you want the average number of items per "paid" order then query 1 applies:
select
	order_id,
    avg(quantity) as average_items
from order_items
where order_id in (
	select order_id
    from orders
...
-- If you want the average number of items per order that is "paid" then query 2 applies:
select
